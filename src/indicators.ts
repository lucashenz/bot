import { getCandles } from './hyperliquidClient';
...
function calculateStdDev(candles: Candle[], period: number): number {
...
function calculateBBPercent(candles: Candle[], period: number = 20, stdDev: number = 2): number {
...
export async function checkSignals(): Promise<'long' | 'short' | null> {
  try {
    const candles = await getCandles(config.timeframe, 250);
