...
async function main() {
    ...
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    console.log(`🌐 Network:          ${config_1.config.isTestnet ? 'TESTNET ⚠️' : 'MAINNET 🔴'}`);
    console.log(`📊 Par:              ${config_1.config.symbol}`);
    console.log(`💰 Tamanho posição:  ${config_1.config.positionSize} BTC`);
    console.log(`📈 Alavancagem:      ${config_1.config.leverage}x`);
    console.log(`🎯 Take Profit:      ${config_1.config.takeProfitPct}%`);
    console.log(`🛡️  Stop Loss:        ${config_1.config.stopLossPct}%`);
    console.log(`⏰ Timeframe:        ${config_1.config.timeframe}`);
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    console.log('');
    if (!config_1.config.isTestnet) {
    ...
    await (0, hyperliquidClient_1.subscribeWebSocket)((price) => {
        currentPrice = price;
    });
...
async function executeStrategy() {
    ...
    try {
        ...
        const signal = await (0, indicators_1.checkSignals)();
        if (!signal) {
            return;
        }
        console.log(`\n💡 Verificando posição atual...`);
        const pos = await (0, hyperliquidClient_1.getOpenPosition)();
        if (pos.hasPosition) {
        ...
        if (signal === 'long') {
            if (pos.hasPosition && !pos.isLong) {
                ...
                await (0, hyperliquidClient_1.closePosition)();
                await new Promise(resolve => setTimeout(resolve, 2000));
            }
            if (!pos.hasPosition) {
                if (currentPrice > 0) {
                    console.log('\n📈 Executando ordem de COMPRA (LONG)...\n');
                    await (0, hyperliquidClient_1.placeOrder)(true, config_1.config.positionSize, config_1.config.leverage, currentPrice);
                }
                else {
            ...
            if (!pos.hasPosition) {
                if (currentPrice > 0) {
                    ...
                    await (0, hyperliquidClient_1.placeOrder)(false, config_1.config.positionSize, config_1.config.leverage, currentPrice);
                }
                else {
                    console.log('❌ Preço inválido, aguardando próxima verificação\n');
                }
